...
namespace Coop.Mod.Vendors
{
    public abstract class NoHarmonyLoader : MBSubModuleBase
    {
        ...
        private readonly List<BehaviorDelegate> _behaviorDelegates = new List<BehaviorDelegate>();
        private readonly List<TaskMode> _behaviorModes = new List<TaskMode>();

        private readonly List<ModelDelegate> _modelDelegates = new List<ModelDelegate>();
        private readonly List<TaskMode> _modelModes = new List<TaskMode>();
        private readonly List<TaskStatus> _tsBehaviors = new List<TaskStatus>();
        private readonly List<TaskStatus> _tsModels = new List<TaskStatus>();
        ...
        protected override void OnSubModuleLoad()
        {
            ...
            using (myLock.Acquire())
            {
                ...
                Log(LogLvl.Info,
                    "Pending tasks : " +
                    _modelDelegates.Count +
                    " models, " +
                    _behaviorDelegates.Count +
                    " behaviors.");
            }
        }
        ...
        protected override void OnGameStart(Game game, IGameStarter gameStarterObject)
        {
            ...
            if (_modelDelegates.Count != _modelModes.Count)
            {
                Log(
                    LogLvl.Error,
            ...
            for (int index = 0; index < _modelDelegates.Count; ++index)
            {
                TaskStatus result =
                    _modelDelegates[index].Invoke(gameStarterObject, _modelModes[index]);
                _tsModels.Add(result);
            }
        }
        ...
        public override void OnGameInitializationFinished(Game game)
        {
            ...
            if (_behaviorDelegates.Count != _behaviorModes.Count)
            {
                Log(
                    LogLvl.Error,
            ...
            for (int index = 0; index < _behaviorDelegates.Count; ++index)
            {
                TaskStatus result = _behaviorDelegates[index].Invoke(campaign, _behaviorModes[index]);
                _tsBehaviors.Add(result);
            }
        }
        ...
        {
            _modelDelegates.Add(NHLModel<AddType, GameModel>);
            _modelModes.Add(TaskMode.Add);
        }
            ...
            where RemoveType : GameModel
        {
            ...
            _modelDelegates.Add(NHLModel<AddType, RemoveType>);
            _modelModes.Add(m);
        }
        ...
        public void RemoveModel<RemoveType>()
            where RemoveType : GameModel
        {
            _modelDelegates.Add(NHLModel<DummyModel, RemoveType>);
            _modelModes.Add(TaskMode.Remove);
        }
        ...
        {
            _behaviorDelegates.Add(NHLBehavior<AddType, CampaignBehaviorBase>);
            _behaviorModes.Add(TaskMode.Add);
        }
        ...
        {
            _behaviorDelegates.Add(NHLBehavior<AddType, RemoveType>);
            _behaviorModes.Add(m);
        }
